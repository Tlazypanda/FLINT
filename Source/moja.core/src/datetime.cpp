...
// The AddMonths method calculates the resulting month and year, taking into account leap years and the number of days
// in a month, then adjusts the day part of the resulting DateTime object.If the resulting day is not a valid day in the
// resulting month, the last valid day of the resulting month is used. For example, March 31st + 1 month = April 30th,
// and March 31st - 1 month = February 28 for a non - leap year and February 29 for a leap year. The time - of - day
// part of the resulting DateTime object remains the same as this instance.

DateTime& DateTime::addMonths(int months) {
   _date += date::months(months);
   if (!_date.ok()) _date = _date.year() / _date.month() / last;
   return *this;
}

DateTime& DateTime::addDays(double days) {
   auto tp =
       sys_days(_date) + _tod.to_duration() + duration_cast<microseconds>(duration<double, date::days::period>(days));
